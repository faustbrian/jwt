...
test('assert should raise exception when claim is not set', function (): void {
    ...
    new HasClaimWithValue('claimId', 'claimValue')->assert(hasClaimWithValueToken());
});

test('assert should raise exception when claim value does not match', function (): void {
    $this->expectException(ConstraintViolation::class);
    $this->expectExceptionMessage('The claim "claimId" does not have the expected value');

    new HasClaimWithValue('claimId', 'claimValue')->assert(
        hasClaimWithValueToken(['claimId' => 'Some wrong value']),
    );
});
...
test('assert should raise exception when token is not a plain token', function (): void {
    ...
    new HasClaimWithValue('claimId', 'claimValue')->assert($this->createStub(TokenInterface::class));
});

test('assert should not raise exception when claim matches', function (): void {
    new HasClaimWithValue('claimId', 'claimValue')->assert(
        hasClaimWithValueToken(['claimId' => 'claimValue']),
    );

    $this->addToAssertionCount(1);
});

function hasClaimWithValueToken(array $claims = [], array $headers = [], ?Signature $signature = null): Plain
{
    return new Plain(
        new Headers($headers, ''),
        new Claims($claims, ''),
